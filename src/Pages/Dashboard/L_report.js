...
const L_report = () => {
    ...
    const handleAdded = event => {
        ...
        fetch(`http://202.164.213.67/mat/accept_all_data.php?DEPT_CODE=${type}&& DATE_=${date}`)
        ...
        <div>
            <div className='flex justify-center mt-4 '>
                <div className='p-5 sm:h-full shadow-2xl lg:w-3/1 lg:h-full  bordered lg:ml-12 lg:p-8 '>
                    <h3 className='font-bold text-md text-secondary drop-shadow mb-2'>SHORT LEAVE <span className='text-primary text-xl'>REPORT DATE WISE</span> </h3>
...

            {
                l_data.length === 0 ?
                    ...
                    :
                    l_data.map(leave => <Report key={leave.id} leave={leave}></Report>)
